<template>
  <div class="flex flex-row xl:w-96 min-h-screen bg-secondary">
    ...
    <button @click="toggleSidebar"
      :class="['fixed top-4 left-4 z-50 p-2 bg-gray-100 rounded-lg lg:hidden transform transition-transform duration-300 ease-in-out', isSidebarOpen ? 'translate-x-2' : '-translate-x-1 lg:translate-x-0']">
    ...
    <!-- Sidebar -->
    <div
      :class="['flex bg-secondary flex-col gap-4 border-zinc-800 border  min-h-screen overflow-y-auto w-96 fixed transform transition-transform duration-300 ease-in-out', isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0']">
      <div class="flex flex-col gap-2 h-screen overflow-y-auto scrollbar-hidden">
        <div class="flex justify-between w-full items-center border border-zinc-800 p-3">
          <div></div>
          <p class="text-white font-bold text-[14px] ">ALL NOTE </p>
          <div>
        ...
        <div v-else-if="sortedNotes.length" class="flex p-3 flex-col gap-4">
          <div v-for="note in sortedNotes" :key="note._id" @click="selectNote(note)">
            ...
            <router-link v-if="note._id" :to="`/home/detail/${note._id}`" :class="[
              'cursor-pointer w-full h-24 rounded-lg px-2 py-3 flex flex-col shadow-md transition-all',
              routee.params.id === note._id ? 'bg-zinc-800' : 'bg-secondary'
            ]">


              <p class="text-[12px] text-white font-bold font-sans">{{ note.title }}</p>
              <p class="text-white text-[12px] line-clamp-2">{{ note.content }}</p>
